...
// end WITH_TAINT_TRACKING
    ...
    protected void sendSms(SmsTracker tracker) {
        ...
        byte pddu[] = (byte[]) map.get("pdu");
        
// begin WITH_TAINT_TRACKING
        int tag = Taint.getTaintByteArray(pdu);
  
        System.out.println("&&&&&&&&&&&&&&&&&  Tag &&&&&&&&& "+ tag);
        if((tag & Taint.NO_TAINT) != Taint.NO_TAINT){
        	if (tag != Taint.TAINT_CLEAR) {
        		...
        		throw new RuntimeException("Insecure app. Message not sent");
        	}
        	
        }
        else{
        	System.out.println("!!!!  Data was Declassified !!!!");
            Taint.setNewTaintByteArray(pdu,Taint.TAINT_CLEAR);
            tracker.mData.put("pdu",pdu);
            System.out.println("%%%%%%%%%%%%%% Tracker pdu tag now "+Taint.getTaintByteArray((byte[])tracker.mData.get("pdu")));
        }
